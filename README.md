...
| 表被仕様 | coating_type | text | ○ |
| 数量 | quantity | integer | ○ |
| メモ | memo | text | |
| 更新日 | updated_at | timestamp | ○ |

※primary keyはid <br>
※symbol, diameter, thickness, coating_type の組み合わせで一意制約を設定（重複防止） <br>
...
※primary keyはid <br>
※symbol, gauge, color の組み合わせで一意制約を設定（重複防止） <br>
※updated_atは登録時または更新時に自動入力 
...
- シンプルな操作
- ボタンの活性非活性の切り替えを設定
- 全ての入力欄で入力方法を指定(選択肢、数字・英数字のみ、4桁まで)
